...
public class GeneticAlgorithm {
	//开始遗传
    public Specieslndividual run(SpeciesPopulation list)
    {
        //创建初始种群
    ...
    {
        //100%随机
        int randomNum=(int)(TSPData.SPECIEN_NUM);
        for(int i=1;i<=randomNum;i++)
        {
            Specieslndividual species=new Specieslndividual();//创建结点
...
//        }
    ...
    void calRate(SpeciesPopulation list)
    {
        ...
        list.speciesNum=0;
        Specieslndividual point=list.head.next;//游标
    ...
    {           
        ...
        Specieslndividual talentSpecies=null;
        Specieslndividual point=list.head.next;//游标
        ...
        {
            //复制物种至新表
            Specieslndividual newSpecies=talentSpecies.clone();
        ...
        for(int i=1;i<=roundNum;i++)
        {
            ...
            Specieslndividual oldPoint=list.head.next;//游标
            while(oldPoint != null && oldPoint != talentSpecies)//寻找表尾结点
            {
                if(rate <= oldPoint.rate)
                {
                    Specieslndividual newSpecies=oldPoint.clone();
                    newSpeciesPopulation.add(newSpecies);
            ...
            if(oldPoint == null || oldPoint == talentSpecies)
            {
                ...
                while(point.next != null)//寻找表尾结点
                    point=point.next;
                Specieslndividual newSpecies=point.clone();
                newSpeciesPopulation.add(newSpecies);
            }
    ...
    {   
        //每一物种均有变异的机会,以概率pm进行
        Specieslndividual point=list.head.next;
        while(point != null)
        {
            float rate=(float)Math.random();
            if(rate < TSPData.pm)
    ...
    //获得适应度最大的物种
    Specieslndividual getBest(SpeciesPopulation list)
    {
        float distance=Float.MAX_VALUE;
        Specieslndividual bestSpecies=null;
        Specieslndividual point=list.head.next;//游标
        while(point != null)//寻找表尾结点
        {
